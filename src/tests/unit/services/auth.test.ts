...
describe("auth register", () => {
  ...
  it("throws when phone number already exists", async () => {
    mockFindUnique.mockResolvedValue({ id: "user-1", phoneNumber: "250788111111" });
    await expect(
      register({ phoneNumber: "250788111111", fullName: "Test" })
  ...
  it("normalizes phone (trims whitespace)", async () => {
    ...
    expect(mockFindUnique).toHaveBeenCalledWith({ where: { phoneNumber: "250788123456" } });
  });
