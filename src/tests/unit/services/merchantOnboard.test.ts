...
jest.mock("../../../db", () => ({
  prisma: {
    merchantCategory: { findUnique: (...args: unknown[]) => mockCategoryFindUnique(...args) },
    user: {
      findUnique: (...args: unknown[]) => mockUserFindUnique(...args),
      upsert: (...args: unknown[]) => mockUserUpsert(...args),
    },
    account: { create: (...args: unknown[]) => mockAccountCreate(...args) },
    merchantProfile: { create: (...args: unknown[]) => mockProfileCreate(...args) },
  },
}));
...
describe("merchantOnboard onboardMerchant", () => {
  ...
  it("throws when user already has a merchant account", async () => {
    mockCategoryFindUnique.mockResolvedValue({ id: "cat-1", code: "GROCERIES" });
  ...
  it("allows onboarding when user exists but has no merchant account", async () => {
    ...
    mockUserUpsert.mockResolvedValue({ id: "user-1", phoneNumber: "250788", fullName: "Gas Co" });
    mockAccountCreate.mockResolvedValue({ id: "acc-m", userId: "user-1", type: "MERCHANT" });
    mockProfileCreate.mockResolvedValue({ id: "prof-1", accountId: "acc-m", categoryId: "cat-1", businessName: "Gas Co" });
