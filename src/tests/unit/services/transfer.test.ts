...
describe("p2pTransfer", () => {
  ...
  it("throws when recipient not found", async () => {
    ...
    await expect(
      p2pTransfer({ fromUserId: "u1", toPhoneNumber: "250788999999", amount: 100 })
    ).rejects.toThrow("Recipient not found");
  });

  it("throws when transferring to self", async () => {
    mockUserFindUnique.mockResolvedValue({ id: "u1", phoneNumber: "250788123456" });
    await expect(
      p2pTransfer({ fromUserId: "u1", toPhoneNumber: "250788123456", amount: 100 })
    ).rejects.toThrow("Cannot transfer to yourself");
  });

  it("throws when insufficient balance", async () => {
    mockUserFindUnique.mockResolvedValue({ id: "u2", phoneNumber: "250788123457" });
    ...
    await expect(
      p2pTransfer({ fromUserId: "u1", toPhoneNumber: "250788123457", amount: 100 })
