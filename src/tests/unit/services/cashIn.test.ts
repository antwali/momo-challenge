...
describe("cashIn", () => {
  ...
  it("throws when amount <= 0", async () => {
    ...
    await expect(cashIn({ agentCode: "A1", userPhoneNumber: "250788", amount: -1 })).rejects.toThrow(
      "Amount must be positive"
    );
  ...
  it("throws when agent invalid or inactive", async () => {
    ...
    await expect(
      cashIn({ agentCode: "BAD", userPhoneNumber: "250788123456", amount: 100 })
  ...
  it("credits account and returns result", async () => {
    mockAgentFindUnique.mockResolvedValue({ id: "agent-1", code: "A1" });
    mockUserFindUnique.mockResolvedValue({ id: "user-1", phoneNumber: "250788123456" });
